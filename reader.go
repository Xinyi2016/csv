...
func (r *Reader) Read(fmts ...Format) (record []string, err error) {
	if r.ReuseRecord {
		record, err = r.readRecord(r.lastRecord, fmts...)
		r.lastRecord = record
	} else {
		record, err = r.readRecord(nil, fmts...)
	}
	return record, err
}
...
// reported.
func (r *Reader) ReadAll(fmts ...Format) (records [][]string, err error) {
	for {
		record, err := r.readRecord(nil, fmts...)
		if err == io.EOF {
			return records, nil
